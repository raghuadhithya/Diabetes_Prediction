...
def submit():
    ...
    Ageval=Age.get()
    print(Ageval)
    if(Glucoseval!=0 and BloodPressureval!=0 and SkinThicknessval!=0 and Insulinval!=0 and BMIval!=0 and DiabetesPedigreeFunctionval!=0 and Ageval!=0):     
...
#  print(round(max(result_percentage[0])*100,2),"% it is ", result[0])
    ...
    #print(prob," % ",result[0])
        ...
        ms.showinfo('Invalid Error', 'Error: Enter all data!')
...
btn1=Button(app,command=submit,text="submit",font=("Georgia",16),bg="green",fg="white",activebackground="red",activeforeground="yellow",width=10,bd=3)
